...
export class Mastodon2Provider extends SocialAbstract implements SocialProvider {
  override maxConcurrentJob = 5; // Mastodon instances typically have generous limits
  identifier = 'mastodon2';
  name = 'Mastodon2';
  ...
  protected generateUrlDynamic(
    ...
    return `${customUrl}/oauth/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(
      `${url}/integrations/social/mastodon2`
  ...
  async generateAuthUrl() {
    ...
    const url = this.generateUrlDynamic(
      process.env.MASTODON2_URL || 'https://mastodon.social',
      state,
      process.env.MASTODON2_CLIENT_ID!,
  ...
  protected async dynamicAuthenticate(
    ...
    form.append(
      'redirect_uri',
      `${process.env.FRONTEND_URL}/integrations/social/mastodon2`
    );
  ...
  async authenticate(params: {
    ...
    return this.dynamicAuthenticate(
      process.env.MASTODON2_CLIENT_ID!,
      process.env.MASTODON2_CLIENT_SECRET!,
      process.env.MASTODON2_URL || 'https://mastodon.social',
      params.code
    );
  }
